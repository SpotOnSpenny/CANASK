...
- [ ] **Set the site admin key after deploy**: run `make prod-rotate-site-admin-key`
      once the stack is up (after `db upgrade` creates the `site_admin_key` table). The initial set
      is CLI-only — until it runs, the gated site-admin flows refuse with a
